...
def loop_a():
    ...
    registered_users = {}
    count=0     #total number of messages
    numRed=0;
    numBlue=0
    ...
    while True:
        if len(registered_users)>0 and count%push_to_arduino(len(registered_users))==1:
            a=mvmt_val.democracy_val(commands_list,0)
            b=mvmt_val.democracy_val(commands_list,1)
            print str([a,b])
            ser.write(struct.pack('>BB', a,b))
            commands_list=[[0,0,0,0],[0,0,0,0]]      #resets commands list
            count+=1
                ...
                #WIN CONDITION!!
                    ...
                    if registered_users.has_key(username):
                        ...
                        if command_lookup.has_key(message):
                            ...
                            commands_list[registered_users.get(username)][command_lookup.get(message,0)]+=1
